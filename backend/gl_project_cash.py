...
async def project_resolver(org_id=ORG_ID):
    ...
    for coll in VIA_DEAL_COLLECTIONS:
        try:
            ...
            logger.debug("koleksi %s dilewati", coll, exc_info=True)
    payments = {}
    for r in await db.payments_out.find({"org_id": org_id},
                                        {"_id": 0, "id": 1, "bill_id": 1}).to_list(50000):
        if r.get("bill_id") and r["bill_id"] in direct:
            payments[r["id"]] = direct[r["bill_id"]]
