...
def npwp_state(value) -> tuple:
    """(ok, detail) — Coretax menuntut 16 digit; 15 digit lama harus ditulis dengan 0 di depan."""
    d = npwp_digits(value)
    if not d:
        return False, "NPWP pembeli belum diisi"
    if len(d) == NPWP_LEN:
        return True, ""
    if len(d) == 15:
        return False, (f"NPWP {d} masih 15 digit — Coretax menuntut 16 digit "
                       f"(tulis 0{d})")
    return False, f"NPWP {d} panjangnya {len(d)} digit, seharusnya {NPWP_LEN} digit"
...
async def _company(org_id) -> dict:
    ...
    return {"npwp": npwp_digits(npwp), "idtku": str(idtku or ""),
...
async def replace_faktur(org_id, faktur_id: str, actor: str, reason: str,
    ...
    doc = {
        **{k: v for k, v in old.items() if k not in ("id", "number", "status", "created_at")},
        "id": __import__("core_utils").new_id(),
...
async def export_check(org_id=ORG_ID, period: str = None) -> dict:
    ...
    rows = data["rows"]
    blocking = []
    if not company["npwp"] or len(company["npwp"]) != NPWP_LEN:
        blocking.append({"scope": "perusahaan", "number": None,
                         "reason": ("NPWP perusahaan belum diisi 16 digit di Pusat Konfigurasi → "
                                    "Pajak & Kepatuhan — berkas ekspor tidak boleh dibuat tanpa "
                                    "identitas pemungut.")})
    for r in rows:
        if not r["npwp_ok"]:
...
def _xml(rows: list, company: dict) -> bytes:
    ...
    for r in rows:
        ...
        parts += [
            ...
            f"      <BuyerTin>{escape(npwp_digits(r.get('buyer_npwp')))}</BuyerTin>",
            "      <BuyerDocument>TIN</BuyerDocument>",
            f"      <BuyerName>{escape(str(r.get('buyer_name') or ''))}</BuyerName>",
            f"      <BuyerIDTKU>{escape(npwp_digits(r.get('buyer_npwp')))}</BuyerIDTKU>",
...
def _csv(rows: list, company: dict) -> bytes:
    ...
    for r in rows:
        w.writerow([r.get("number"), str(r.get("issued_at") or "")[:10],
                    ...
                    npwp_digits(r.get("buyer_npwp")), r.get("buyer_name"),
...
async def vat_return(org_id=ORG_ID, period: str = None) -> dict:
    ...
    fak = await list_faktur(org_id, period=period, status=ACTIVE)
